...
def set_keyframes(obj, frame_start, frame_end, loc_start, rot_start, loc_end, rot_end):
    # Set location and rotation at the start frame
    bpy.context.scene.frame_set(frame_start)
    obj.location = loc_start
    obj.rotation_euler = rot_start
    
    # Insert keyframes for location and rotation at the start frame
    obj.keyframe_insert(data_path="location", index=-1)
    obj.keyframe_insert(data_path="rotation_euler", index=-1)

    # Set location and rotation at the end frame
    bpy.context.scene.frame_set(frame_end)
    obj.location = loc_end
    obj.rotation_euler = rot_end
    
    # Insert keyframes for location and rotation at the end frame
    obj.keyframe_insert(data_path="location", index=-1)
    obj.keyframe_insert(data_path="rotation_euler", index=-1)
# File path to the name file
name_file_path = r"C:\Users\sunil\OneDrive\Desktop\Db2 Challenges\organise\Beuatiful\placement_19july\project1_a\name_output.txt"
def set_keyframes_for_object(obj_name, coord_file_path):
    # Read final coordinates from the specified file
    try:
        with open(coord_file_path, 'r') as file:
            # Assuming the first line contains the coordinates in the format: X, Y
            coord_line = file.readline().strip()
            X, Y = map(float, coord_line.strip('()').split(','))  # Convert to float
    ...
    # Set locations and rotations for keyframes
    loc_start = (0, 0.20759, 4.0575)
    rot_start = (math.radians(-14.669), 0, 0)  # Start rotation in radians

    loc_end = (X, Y, 0.6)
    rot_end = (math.radians(-5.1113), 0, 0)  # End rotation in radians

    # Set keyframes from frame 0 to frame 104
    set_keyframes(obj, 0, 104, loc_start, rot_start, loc_end, rot_end)
...
image_path = r"C:\Users\sunil\OneDrive\Desktop\Db2 Challenges\organise\Beuatiful\placement_19july\python_july19\simple7.jpg"
...
if plane is None:
    ...
    mapping_node = nodes.new(type='ShaderNodeMapping')
    tex_coord_node = nodes.new(type='ShaderNodeTexCoord')
    image_path = r"C:\Users\sunil\OneDrive\Desktop\Db2 Challenges\organise\Beuatiful\placement_19july\python_july19\simple8.jpg"
...
input_file_path = r"C:\Users\sunil\OneDrive\Desktop\Db2 Challenges\organise\Beuatiful\placement_19july\project1_a\non_name_output.txt"
...
text_obj.data.size = 0.7

font_path = r"C:\Users\sunil\Downloads\almonte\Almonte Snow.otf"

# Load the new font
bpy.ops.font.open(filepath=font_path)

# Ensure you are referencing the correct font after loading it
new_font = bpy.data.fonts.load(font_path)

# Now assign the font to your text object
text_obj = bpy.context.object  # Make sure you have a text object selected
text_obj.data.font = new_font
...
file_path = r"C:\Users\sunil\OneDrive\Desktop\Db2 Challenges\organise\Beuatiful\placement_19july\project1_a\non_name_coordinates.txt"

# Read the file and extract X and Y
...
# Function to determine if the face normal is primarily in the Z direction
def is_z_direction(normal):
    return abs(normal.z) > abs(normal.x) and abs(normal.z) > abs(normal.y)

# Function to create a text object and assign properties and materials
def create_text_object(input_text, font_path):
    # Add a text object
    bpy.ops.object.text_add()
    text_obj = bpy.context.active_object
    ...
    # Ensure materials are assigned if they already exist
    material_z = bpy.data.materials.get("Material_Z")
    material_other = bpy.data.materials.get("Material_Other")
    
    if len(text_obj.data.materials) == 0:
        # Add materials if the list is empty
        text_obj.data.materials.append(mat2)
        text_obj.data.materials.append(material)
    else:
        # Use existing materials
        text_obj.data.materials[0] = mat2
        text_obj.data.materials[1] = material

# Read the name from the file
name_file_path = r"C:\Users\sunil\OneDrive\Desktop\Db2 Challenges\organise\Beuatiful\placement_19july\project1_a\name_output.txt"
non_name_file_path = r"C:\Users\sunil\OneDrive\Desktop\Db2 Challenges\organise\Beuatiful\placement_19july\project1_a\non_name_output.txt"
...
try:
    with open(name_file_path, 'r') as file:
        ...
        name_line = file.readline().strip()
        # Extract name from the line; modify as necessary based on your file format
        name = name_line.split(" ")[0]  # Adjust this based on how the name is stored

    # Font path for the text object
    font_path = r"C:\Users\sunil\Downloads\almonte\Almonte Snow.otf"
    
    # Create the text object with the name read from the file
    create_text_object(name, font_path)
    coord_file_path = r"C:\Users\sunil\OneDrive\Desktop\Db2 Challenges\organise\Beuatiful\placement_19july\project1_a\name_coordinates.txt"
    
    # Set keyframes for the object "StyledText.001"
    set_keyframes_for_object("StyledText.001", coord_file_path)

except Exception as e:
    print(f"An error occurred while reading the name file: {e}")
